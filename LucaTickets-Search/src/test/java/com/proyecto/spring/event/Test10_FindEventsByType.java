...
public class Test10_FindEventsByType {
	...
	void contextLoads() throws Exception {
		...
		when(sc.getEventByType("terraza")).thenReturn((List<EventResponse>) eventResponse);
		
		mockMvc
			.perform(get("http://localhost:7777/event/"))
			.andDo(print())
			.andExpect(status().isOk())
			.andExpect(content().string(containsString("terraza")));
	}
}
